...
/*   Updated: 2021/02/09 17:16:19 by eluceon          ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
int		ft_check_line_error(t_all *all, char *line)
{
	...
	if (all->win.width == 0 || all->win.height == 0)
		ft_exit(all, -12, 1);
	if ((all->color.ceiling < 0 || all->color.floor < 0))
		ft_exit(all, -6, 1);
	i = -1;
	while (++i < TEXTURES)
	{
...
int		ft_parse_manager(t_all *all, char *line, t_list **head, int i)
{
	if (ft_isemtyline(line) && !(*head))
		...
		return (ft_parse_colors(line, i, &all->color.floor));
	else if (line[i] == 'C' && line[i + 1] == ' ')
		return (ft_parse_colors(line, i, &all->color.ceiling));
